...
function generateAutoReplyHTML(data, language) {
  ...
  const greetings = {
    contact: {
      JP: { greeting: 'お問合せありがとうございます。', received: '以下のお問合せを受け付けました。' },
      EN: { greeting: 'Thank you for your inquiry.', received: 'We have received the following information:' },
      KS: { greeting: '문의해 주셔서 감사합니다.', received: '다음과 같이 문의를 접수하였습니다:' },
      CN: { greeting: '感谢您的咨询。', received: '我们已收到以下咨询：' }
    },
    reserve: {
      JP: { greeting: 'ご予約ありがとうございます。', received: '以下の予約内容を受け付けました。' },
      EN: { greeting: 'Thank you for your booking.', received: 'We have received the following booking information:' },
      KS: { greeting: '예약해 주셔서 감사합니다。', received: '다음과 같이 예약을 접수하였습니다:' },
      CN: { greeting: '感谢您的预订。', received: '我们已收到以下预订信息：' }
    }
  };
  
  const currentGreeting = greetings[formType][language] || greetings['contact']['JP'];
  
  const messages = {
    JP: {
      greeting: currentGreeting.greeting,
      received: currentGreeting.received,
      ...
      paymentMethod: 'お支払い方法',
      remarks: '備考',
      cruiseSchedule: 'クルーズ希望日程',
      applicantInfo: '申込者様情報',
      nights: '泊',
      people: '名'
    },
    EN: {
      greeting: currentGreeting.greeting,
      received: currentGreeting.received,
      note: formType === 'reserve' 
        ? 'Please note that depending on availability and your request details, it may take some time for us to confirm.'
    ...
    KS: {
      ...
      received: currentGreeting.received,
      note: formType === 'reserve'
        ? '예약 상황과 요청 내용에 따라 확인까지 시간이 걸릴 수 있습니다.'
        : '문의 상황이나 질문 내용에 따라 답변을 드리는 데 시간이 걸릴 수 있습니다.',
    ...
    CN: {
      ...
      received: currentGreeting.received,
      note: formType === 'reserve'
        ? '根据预订情况和您的要求，确认可能需要一些时间。'
        : '根据咨询情况和问题内容，回复可能需要一些时间。',
      understand: '敬请谅解。',
      inquiryType: '咨询项目',
      name: '姓名',
      furigana: '假名',
...
<head>
  ...
  <style>
    ...
    .content { padding: 40px 30px; }
    .greeting { font-size: 16px; margin-bottom: 20px; line-height: 1.8; }
    .section-title { background: #0698ba; color: white; padding: 12px 20px; border-radius: 6px; margin: 25px 0 15px 0; font-weight: 600; font-size: 15px; }
    .divider { border-top: 2px solid #e0e0e0; margin: 30px 0; }
    .field { margin-bottom: 20px; padding-bottom: 15px; border-bottom: 1px solid #f0f0f0; }
    .field:last-of-type { border-bottom: none; }
    .field-label { font-weight: 600; color: #0698ba; font-size: 13px; margin-bottom: 8px; }
    .field-value { color: #333; font-size: 15px; word-wrap: break-word; white-space: pre-wrap; }
    .remarks-box { background: #f8f9fa; padding: 20px; border-radius: 8px; border-left: 4px solid #0698ba; margin-top: 10px; }
    .footer { background: #f8f9fa; padding: 30px; text-align: center; border-top: 1px solid #e0e0e0; }
    .footer-divider { width: 60px; height: 2px; background: #0698ba; margin: 20px auto; }
    .company-name { font-size: 20px; font-weight: 600; color: #0698ba; margin-bottom: 15px; }
    .contact-info { font-size: 14px; color: #666; line-height: 1.8; }
...
<body>
  <div class="container">
    ...
    <div class="content">
      ...
      <div class="section-title">◆ ${msg.cruiseSchedule || 'クルーズ希望日程'}</div>
      
      <div class="field">
        <div class="field-label">■ ${msg.programName}</div>
        <div class="field-value">${programName || '-'}</div>
      </div>
      
      <div class="field">
        <div class="field-label">■ ${msg.boardingDate}</div>
        <div class="field-value">${boardingDate || '-'}</div>
      </div>
      
      <div class="field">
        <div class="field-label">■ ${msg.stayDays}</div>
        <div class="field-value">${stayDays ? stayDays + (msg.nights || '泊') : '-'}</div>
      </div>
      
      <div class="field">
        <div class="field-label">■ ${msg.members}</div>
        <div class="field-value">${members ? members + (msg.people || '名') : '-'}</div>
      </div>
      
      <div class="section-title">◆ ${msg.applicantInfo || '申込者様情報'}</div>
      ...
      ${gender ? `
      <div class="field">
        <div class="field-label">■ ${msg.gender}</div>
      ...
      ${birthDate ? `
      <div class="field">
        <div class="field-label">■ ${msg.birthDate}</div>
      ...
      ${paymentMethod ? `
      <div class="section-title">◆ ${msg.paymentMethod}</div>
      <div class="field">
      ...
      ${remarks ? `
      <div class="section-title">◆ ${msg.remarks}</div>
      <div class="field">
        <div class="remarks-box">
